...
| `20260905000100` | 人员别名、任务关系别名、解析与管理 RPC；冷启阿部/成研到 C 组成妍 |
| `20260905000200` | 可幂等、可部分成功的导入批次与行级解析/重试字段 |
| `20260905000300` | 字段来源与人工锁、任务事件、参与人快照、数据库权威待办 |
| `20260905000400` | 五状态任务状态机、历史状态迁移和合法流转 RPC |
| `20260905000500` | 任务台账逐行提交、重复建单保护、人工锁冲突和组长待完善待办 |
| `20260905000600` | 组长完善任务字段、主负责人确认和成员任务通知 RPC |
| `20260905000700` | 文档审核待办同步、结项交付物/回扫/贡献门禁 |
| `20260905000800` | 时间点状态、完成工作量、人员工作和审核队列统一报表 RPC |
| `20260905000900` | 任务台账权威服务端预览及行级人工解决 RPC |
| `20260905001000` | 管理员/组长/组员读取边界与已审核/已引用文件删除保护 |
| `20260905001100` | 手工建单受控 RPC，复用关系、参与人、来源、事件和待办契约 |
| `20260905001200` | 回扫台账候选匹配、人工确认、幂等提交与原始行追溯 |
| `20260905001300` | 冲突/失败任务导入行创建独立重试批次和不可变重试链 |
| `20260905001400` | 禁止直接更新任务主状态，强制使用状态/完善/结项 RPC |
| `20260905001500` | 文档路线不可降级、驳回意见必填、组员上传范围与真实上传人保护 |
| `20260905001600` | 客观工作标签目录、参与人约束、证据有效性与受控确认/移除 RPC |
| `20260905001700` | 文档版本链待办闭环与管理员驳回后的返修跟进待办 |
| `20260905001800` | 任务事实表按角色/任务范围可见，数据验收事件 RPC 与私有文件读取收口 |
| `20260905001900` | 结项实际截止时间、禁用主观工作量输入、按已确认客观标签占比分配人员工作量 |
| `20260905002000` | 台账规则文档保留为任务已有资料链接，并与正式审核交付物口径隔离 |
| `20260907000100` | 真实9月台账动态分组补充：AI试衣图/商品图采集、效果评测-美妆显式归入AI试穿临时任务 |
