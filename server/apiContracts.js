...
const resources = [
  ...
  ['GET', '/tasks', '按当前权限读取任务'],
  ['POST', '/tasks', '组长手动创建任务；需写入预计截止时间、初始难度、参与人及三级任务挂链，禁止预填实际截止时间'],
  ['GET', '/tasks/:id', '读取任务全生命周期档案'],
  ['PATCH', '/tasks/:id/setup', '组长完善导入任务；补齐三级挂链、主负责人、预计截止时间和初始难度后转待开始'],
  ['POST', '/tasks/:id/transitions', '按待完善、待开始、进行中、待确认、已完成状态机流转并写事件'],
  ...
  ['POST', '/tasks/:id/settlement', '组长确认最终星级、实际截止时间和客观参与记录并结项；服务端计算工作量'],
  ...
  ['POST', '/imports/:batchId/rows/:rowId/resolve', '人工解决单行任务关系、参与人、小组、负责人或回扫任务关联；不污染全局字典'],
  ['POST', '/imports/:batchId/commit', '幂等提交选中行；字段齐全进入待开始，需补充字段进入待完善'],
  ['POST', '/imports/:batchId/retry', '复制失败/冲突行形成重试链，保留原始行'],
  ['GET', '/imports/:batchId', '读取导入结果、错误行和待确认项'],
  ['GET', '/imports/:batchId/errors.csv', '导出批次结构化问题与失败明细'],
  ['GET', '/todos', '按当前真实身份读取数据库权威待办'],
  ['POST', '/todos/:id/complete', '完成待办并记录处理人和完成时间'],
  ['GET', '/dashboard/overview', '读取 B/C/D 三组事实型总览'],
  ['GET', '/dashboard/risks', '读取重点风险任务'],
  ['GET', '/dashboard/personal-work', '按成员、小组、日期读取已确认工作量和类型分布'],
  ['GET', '/dashboard/task-status', '按日、周、月、公司季度或任意区间读取区间结束时状态快照'],
  ...
  ['POST', '/integration/tasks/preview', '未来调度系统按 Canonical Task Import V2 预检，不写正式任务'],
  ['POST', '/integration/tasks/upsert', '未来调度系统复用任务导入 RPC 幂等写入基础数据'],
