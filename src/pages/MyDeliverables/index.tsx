...
export default function MyDeliverables() {
  ...
  const { actor } = useActor(); const navigate = useNavigate();
  ...
  const rows = useMemo(() => {
    ...
    return task && (task.assignee === actor.name || task.participantNames?.includes(actor.name) || document.uploader === actor.name);
