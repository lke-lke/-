...
const toTaskRow = (task: Partial<Task>) => {
  const { taskGroup, workNature, teamLeader, dataReporter, dataVolume, platformTaskId, ruleDocLink, docCompleteness, createdAt, alerts, ...rest } = task;
  return {
    ...rest,
...
export async function createTask(task: Omit<Task, 'id' | 'status' | 'progress' | 'docCompleteness' | 'alerts'>): Promise<Task> {
  const newTask: Task = {
    ...task,
    id: `t${Date.now()}`,
